...
const Level3Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const querySnapshot = await getDocs(collection(db, 'quizQuestions', 'level3', 'questions'));
  ...
  const handleNext = () => {
    if (currentQuestion + 1 < questions.length) {
      ...
      localStorage.setItem('level3Score', score.toString());
      ...
      {!isFinished ? (
        <div className="bg-white border shadow-md rounded-lg w-full max-w-2xl">
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <h1 className="text-xl font-semibold">Quiz Level 3</h1>
        ...
        <div className="bg-white border shadow-md rounded-lg w-full max-w-lg p-6 text-center">
          ...
          <p className="text-sm text-gray-600">Kamu berhasil menyelesaikan Level 3! 🚀</p>
